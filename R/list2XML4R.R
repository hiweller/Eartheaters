list2XML4R <- function(list, file="", ind=0){
	...
	for(i in 1:length(list)){
		...
		if(class(list[[name]]) == 'logical' || class(list[[name]]) == 'character' || class(list[[name]]) == 'integer' || class(list[[name]]) == 'vector' || class(list[[name]]) == 'numeric'){
			...
			# OPENING TAG
			str <- c(str, paste0(paste(rep('\t', ind), collapse=''), '<', name, ' type=', type, ' names=', names,' as.numeric=', is.numeric(c(list[[name]])),' >', new_line))
		...
		if(class(list[[name]]) == 'matrix'){
			...
			# OPENING TAG
			str <- c(str, paste0(paste(rep('\t', ind), collapse=''), '<', name, ' type=matrix rownames=', row_names,' colnames=', col_names,' as.numeric=', is.numeric(c(list[[name]])),' >\n'))
