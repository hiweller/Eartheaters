calibrateCameras <- function(img.dir, sq.size, nx, ny, cal.file, corner.dir,
	...
	num.sample.est = 'auto', num.sample.sets = 'auto', num.aspects.sample = NULL, max.sample.optim = 30,
	nlm.calls.max = 20, fit.min.break = 1, objective.min.break = 5, with.circles = FALSE, 
	sample.est = NULL, ...){
	...
	if(print.progress){
		cat(paste0("\t\tNumber of aspects from total that will be sampled calibration coefficient estimation (num.sample.est): ", num.sample.est, "\n"))
	...
	if(estimate_cal_coeffs){
		...
		for(i in 1:num.sample.sets){
			...
			dlt_cal_cam <- dltCalibrateCameras(coor.2d=coor_2d_red[, , cal_sample_sets[[i]], ], nx=3, 
				...
				reduce.grid.dim=FALSE, objective.min.break=objective.min.break, min.views=min_views,
				grid.incl.min=2, nlm.calls.max=nlm.calls.max, ...)
